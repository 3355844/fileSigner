...
public class SignGenService {
	...
	public void createDigitSignature(String signId) {
		...
		signOut = DefaultStreamedContent.builder().contentType("application/octet-stream")
				.name("sign_" + signProcess.getFileName().substring(0, signProcess.getFileName().lastIndexOf('.')))
				.stream(() -> new ByteArrayInputStream(signProcess.getSignature())).build();
	}
